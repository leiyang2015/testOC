...
class vector_ref {
  public:
    ...
    operator vector_ref<_T2>() const {
        ...
        return vector_ref<_T2>((_T2 *) m_data, m_count * sizeof(_T) / sizeof(_T2));
    }
    ...
    vector_ref<_T> cropped(size_t _begin, size_t _count = ~size_t(0)) const {
        if (m_data && _begin + std::max(size_t(0), _count) <= m_count)
            return vector_ref<_T>(m_data + _begin, _count == ~size_t(0) ? m_count - _begin : _count);
        else
            return vector_ref<_T>();
    }
    void retarget(_T const *_d, size_t _s) {
        m_data = _d;
        m_count = _s;
    }
